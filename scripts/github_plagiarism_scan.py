...
def _query_fragment(line: str, *, max_chars: int = 120) -> str:
    tokens = re.findall(r"[A-Za-z_][A-Za-z0-9_]{2,}|[0-9]{3,}", line)
...
def _http_error_warning(exc: error.HTTPError, fingerprint: CodeFingerprint) -> str:
    ...
    if exc.code == 403:
        message += (
            " Public code search may require a personal access token or may have "
            "hit an installation rate limit; set "
