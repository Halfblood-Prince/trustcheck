...
DYNAMIC_ANALYZER_IMAGES = {DEFAULT_DYNAMIC_PYTHON: DEFAULT_DYNAMIC_IMAGE}
...
def _runner_source(
    ...
    *,
    ...
    return (
        ...
        "    lowered = path.lower()\n"
        "    if any(part in lowered for part in ('.pypirc', '.netrc', '.ssh', 'id_rsa', 'credentials', 'keyring')):\n"
        "        add_unique('credential_path_accesses', path)\n"
        "    if any(part in lowered for part in ('sitecustomize.py', 'usercustomize.py', '.pth', 'cron', 'systemd', '.bashrc', '.profile')):\n"
        "        add_unique('persistence_attempts', path)\n"
        "def classify_write(path):\n"
        "    text = str(path)\n"
        "    add_unique('files_modified', text)\n"
        "    suspicious_path(text)\n"
        "    allowed = ('/tmp/trustcheck-venv', '/tmp/trustcheck-wheelhouse', '/tmp/trustcheck-metadata', '/tmp/pip-', '/tmp/tmp')\n"
        ...
        "AUDIT_LOG = pathlib.Path('/tmp/trustcheck-audit.jsonl')\n"
        "def hook(event, args):\n"
        "    if event in {'subprocess.Popen', 'open', 'os.mkdir', 'socket.connect', 'os.putenv', 'os.unsetenv'}:\n"
        "        try:\n"
        "            rendered = [str(item) if not isinstance(item, (list, tuple)) else [str(part) for part in item] for item in args]\n"
        "            with AUDIT_LOG.open('a', encoding='utf-8') as stream:\n"
        "                stream.write(json.dumps({'event': event, 'args': rendered}, sort_keys=True) + '\\\\n')\n"
        "        except Exception:\n"
        "            pass\n"
        ...
        "\"\"\".strip() + \"\\n\", encoding='utf-8')\n"
        "def phase(name, status='passed', classification='passed', failure_type=None, exit_code=None, stdout='', stderr='', error=None):\n"
        "    phases.append({'name': name, 'status': status, 'classification': classification, 'failure_type': failure_type, 'exit_code': exit_code, 'stdout': excerpt(stdout), 'stderr': excerpt(stderr), 'error': error})\n"
        "def failure_type(name, stderr):\n"
        "    text = str(stderr).lower()\n"
        "    if 'backendunavailable' in text or 'no module named' in text or 'build backend' in text:\n"
        ...
        "    phase(name, status='failed', classification='inconclusive', failure_type=kind, exit_code=completed.returncode, stdout=completed.stdout, stderr=completed.stderr, error=f'{name} failed with exit code {completed.returncode}')\n"
        "    return False\n"
        ...
        "        phase('archive_validation', status='failed', classification='inconclusive', failure_type='metadata_invalid', error=str(exc))\n"
        "        return False\n"
        "ok = validate_archive()\n"
        ...
        "    ok = run_phase('metadata_preparation', [python, str(PIP_WRAPPER), 'install', '--dry-run', '--report', str(metadata_dir / 'report.json'), '--no-input', '--disable-pip-version-check', '--no-cache-dir', '--no-deps', '--no-index', '--no-build-isolation', *find_links_args, str(ARTIFACT)])\n"
        "if ok and ARTIFACT.suffix.lower() == '.whl':\n"
        "    phase('wheel_build', status='skipped', classification='passed')\n"
        "elif ok:\n"
        "    wheel_dir.mkdir(parents=True, exist_ok=True)\n"
        "    ok = run_phase('wheel_build', [python, str(PIP_WRAPPER), 'wheel', '--no-input', '--disable-pip-version-check', '--no-cache-dir', '--no-deps', '--no-index', '--no-build-isolation', *find_links_args, '--wheel-dir', str(wheel_dir), str(ARTIFACT)])\n"
        "if ok:\n"
        "    wheels = sorted(wheel_dir.glob('*.whl'))\n"
        "    install_target = str(wheels[0]) if wheels else str(ARTIFACT)\n"
        "    ok = run_phase('wheel_installation', [python, str(PIP_WRAPPER), 'install', '--no-input', '--disable-pip-version-check', '--no-cache-dir', '--no-deps', '--no-index', '--no-build-isolation', *find_links_args, install_target])\n"
        "if ok and CONFIG['import_probe']:\n"
        "    phase('import_probe', status='skipped', classification='inconclusive', failure_type='unsupported_behavior', error='import probe is not enabled until package import targets are policy-defined')\n"
        "else:\n"
        "    phase('import_probe', status='skipped', classification='inconclusive')\n"
        "if ok and CONFIG['entry_point_probe']:\n"
        "    phase('entry_point_probe', status='skipped', classification='inconclusive', failure_type='unsupported_behavior', error='entry point probe is not enabled until command targets are policy-defined')\n"
        "else:\n"
        "    phase('entry_point_probe', status='skipped', classification='inconclusive')\n"
        "merge_audit_log()\n"
        "print(CONFIG['result_prefix'] + json.dumps({'phases': phases, 'evidence': evidence}, sort_keys=True))\n"
