...
class CliFailureHandlingTests(unittest.TestCase):
    def test_network_outage_returns_clean_nonzero_exit_code(self):
        stdout = io.StringIO()
        stderr = io.StringIO()

        with patch("trustcheck.cli.inspect_package", side_effect=PypiClientError("unable to reach PyPI: timed out")):
    ...
    def test_missing_package_returns_clean_nonzero_exit_code(self):
        stdout = io.StringIO()
        stderr = io.StringIO()

        with patch("trustcheck.cli.inspect_package", side_effect=PypiClientError("resource not found: https://pypi.org/pypi/demo/json")):
            with redirect_stdout(stdout), redirect_stderr(stderr):
    ...
    def test_malformed_server_response_returns_clean_nonzero_exit_code(self):
        stdout = io.StringIO()
        stderr = io.StringIO()

        with patch("trustcheck.cli.inspect_package", side_effect=ValueError("missing required provenance fields")):
            with redirect_stdout(stdout), redirect_stderr(stderr):
                exit_code = main(["inspect", "demo"])
