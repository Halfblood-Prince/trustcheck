...
The Snap build first creates a local wheelhouse from hash-locked inputs:

- `requirements/runtime.lock` for runtime dependencies;
- `requirements/snap-build.lock` for pinned build backend tooling.

The `trustcheck` part then installs from that wheelhouse with
`--require-hashes`, `--no-index`, and `--find-links`. Runtime dependencies are
therefore selected from the committed lockfile rather than resolved against a
live package index during installation.

Official PyPI sdists and release assets remain the authoritative source
archives for version discovery. GitHub-generated source archives do not contain
the signed release metadata generated by the packaging workflow; when Git
metadata is absent, Trustcheck source builds deliberately fall back to
`0.0.0+source` unless the release workflow supplies an explicit
`SETUPTOOLS_SCM_PRETEND_VERSION_FOR_TRUSTCHECK` value.
